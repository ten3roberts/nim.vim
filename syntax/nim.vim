...
if exists('g:nim_highlight_all')
    ...
    let g:nim_highlight_builtins     = 1
    let g:nim_highlight_exceptions   = 1
    let g:nim_highlight_space_errors = 1
    let g:nim_highlight_special_vars = 1
endif
...
syn match  nimEscape        +\\[abfnrtvl'"\\]+ contained
...
if g:nim_highlight_builtins == 1
    ...
    syn keyword nimBuiltin await
endif

if g:nim_highlight_exceptions == 1
...
if g:nim_highlight_space_errors == 1
    " trailing whitespace
    syn match   nimSpaceError   display "\s\+$"
    " any tabs are illegal in nim
    syn match   nimSpaceError   display "\t"
endif

if g:nim_highlight_special_vars
...
if v:version >= 508 || !exists('did_nim_syn_inits')
    ...
    if g:nim_highlight_space_errors == 1
        HiLink nimSpaceError    Underlined
    endif
